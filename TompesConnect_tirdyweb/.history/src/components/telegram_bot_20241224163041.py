...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://ee42-24-150-119-126.ngrok-free.app"],  # Укажите корректный домен
...
async def add_csp_header(request, call_next):
    nonce = "random123"  # Генерируйте динамически, если требуется
...
async def preflight_handler(path: str):
    return {
        "status": "success",
        "message": "Preflight request allowed"
    }
