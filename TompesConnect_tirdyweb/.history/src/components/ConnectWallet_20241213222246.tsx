...
const ConnectWallet: React.FC = () => {
  ...
  const saveAddressToServer = async (walletAddress: string, token: string) => {
    try {
      console.log(`Saving wallet address: ${walletAddress}`); // Логируем адрес кошелька
      const response = await fetch("http://localhost:5000/save-wallet", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result.error || "Failed to save wallet address on the server.");
      }

      toast.success(`Wallet address linked successfully for user: ${result.user_id}`);
    } catch (err) {
      toast.error(`Server error: ${(err as Error).message}`);
    }
  };
  ...
  const handleConnect = async (walletType: "metamask" | "trustwallet" | "walletconnect") => {
    ...
    try {
      ...
      if (address) {
        console.log("Wallet Address after connect:", address); // Проверка после подключения
        await saveAddressToServer(address, token);
      } else {
        console.error("Wallet address not found."); // Ошибка, если адрес не найден
      }
    } catch (err) {
